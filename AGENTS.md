...
- Visual source of truth: [`design.md`](design.md) (TailGrids v3). Read it before building UI.
- Use **Tailwind CSS** with the project's **semantic tokens** (`text-text-*`, `bg-card-*`, `border-*`, `bg-brand-*`, etc.).
- Never hardcode hex colors in components — always reference tokens from `src/app/globals.css` that map to `design.md`.
