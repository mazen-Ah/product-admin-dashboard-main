...
This file is the visual source of truth for the product. Tailgrids primitives in `src/components/tailgrids/core/` are the only UI kit.

In code:

- Map these values to tokens in `src/app/globals.css`, `src/app/css/default.css`, and `src/app/css/dark.css`.
- Use semantic Tailwind classes (`text-text-*`, `bg-card-*`, `border-*`, `bg-brand-*`). Never paste hex into components.
- Icons: `@tailgrids/icons` only. Do not introduce Heroicons or a second icon family.
- One filled primary button per view. Status colors are for status only.
- Arabic / RTL will use these same tokens. Pair Inter with an Arabic-capable face when i18n lands; do not invent a second color system.
...
- **Letter spacing:** 0em
- **Role:** Code snippets, technical documentation, inline code, and any data display requiring monospaced alignment. Use for bon numbers, truck plates, wallet balances, and other aligned figures.
...
Bon, settlement, supplier, and wallet grids use this table chrome. Large datasets use TanStack Table on top of these styles — do not introduce a second table look.
...
App shell in this repo: sidebar ~270px expanded / 72px collapsed, content on Gray 50 with a white rounded work surface. Do not introduce a second shell.
...
- **Radix Themes** — Systematic neutral palette, Inter typeface, accessible-first component design with semantic color application.
...
These tokens belong in CSS files, not in component JSX. Prefer the semantic names already mapped in `src/app/globals.css` when a Tailgrids token already exists (`text-text-primary`, `bg-card-surface-area`, `border-card-border`, `bg-brand-500`).
...
:root {
  ...
  --shadow-sm: 0px 1px 3px 0px rgba(9, 14, 52, 0.1), 0px 1px 2px 0px rgba(9, 14, 52, 0.06);
  --shadow-md: 0px 4px 6px -1px rgba(9, 14, 52, 0.1), 0px 2px 4px -1px rgba(9, 14, 52, 0.06);
  --shadow-lg: 0px 10px 15px -3px rgba(9, 14, 52, 0.1), 0px 4px 6px -2px rgba(9, 14, 52, 0.05);
  --shadow-xl: 0px 20px 25px -5px rgba(9, 14, 52, 0.1), 0px 10px 10px -5px rgba(9, 14, 52, 0.04);
  --shadow-2xl: 0px 25px 50px -12px rgba(9, 14, 52, 0.25);
  --shadow-inner: inset 0px 2px 4px 0px rgba(9, 14, 52, 0.06);
  --shadow-focus-primary: 0px 0px 0px 3px rgba(55, 88, 249, 0.2);
